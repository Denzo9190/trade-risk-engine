...
public class BacktestEngine {
    ...
    private final MarketDataAdapter historicalAdapter; // для обновления кэша
    private final PriceCache priceCache;               // кэш для цен
    ...
    public BacktestEngine(TradingStrategy strategy,
                          ...
                          MarketDataAdapter historicalAdapter,
    ...
    public BacktestResult run(String symbol, List<Instant> timeline) {
        for (Instant tick : timeline) {
            ...
            // Обновляем кэш актуальной исторической ценой
            BigDecimal price = historicalAdapter.getPrice(symbol);
            priceCache.put(symbol, price);
            // Генерируем сигнал и исполняем
        ...
        long winningTrades = trades.stream()
                .filter(t -> t.getSide() == Side.SELL && t.getPrice().compareTo(t.getQuantity()) > 0)
                .count();
        ...
        return new BacktestResult(trades, realisedPnl, unrealisedPnl, BigDecimal.ZERO, winRate);
    }
}
